...
## Architektur-Entscheidung: Zwei Abrechnungstypen

### Option: Separate URLs mit Typ-Parameter

```
/abrechnung/2025/preview/0003/eigentuemer  → Hausgeldabrechnung (WEG)
/abrechnung/2025/preview/0003/mieter       → Betriebskostenabrechnung (Mietrecht)
```

### Vergleich der Abrechnungstypen

| Aspekt | Eigentümer (HGA) | Mieter (BKA) |
|--------|------------------|--------------|
| **Rechtsgrundlage** | § 28 WEG, BGH V ZR 271/12 | § 556 BGB, BetrKV |
| **Abrechnungsprinzip** | Zufluss-/Abfluss (Pflicht!) | Periodengerecht erlaubt |
| **Umlagefähige Kosten** | ✅ Ja | ✅ Ja |
| **Nicht umlagefähige Kosten** | ✅ Ja | ❌ Nein |
| **Rücklagenzuführung** | ✅ Separat ausweisen | ❌ Nein |
| **Vermögensbericht** | ✅ Pflicht seit 2020 | ❌ Nein |
| **Hausgeld-Soll/Ist** | ✅ Ja | ❌ Nein (nur Vorauszahlungen) |
| **Kostenglättung** | ❌ Verboten | ✅ Erlaubt |
| **§35a EStG** | ✅ Ja | ✅ Ja |
...
**Route:**
```php
// src/Controller/AbrechnungController.php

#[Route('/abrechnung/{year}/preview/{einheitNummer}/{type}', name: 'abrechnung_preview')]
public function preview(
    int $year,
    string $einheitNummer,
    string $type = 'eigentuemer'  // Default: Eigentümer
): Response {
    $einheit = $this->findEinheit($einheitNummer);

    return match($type) {
        'eigentuemer' => $this->renderEigentuemerAbrechnung($einheit, $year),
        'mieter' => $this->renderMieterAbrechnung($einheit, $year),
        default => throw new NotFoundHttpException("Unknown type: $type"),
    };
}
```
...
**Service-Architektur:**
```
AbrechnungService (Interface)
├── HausgeldabrechnungService      → Eigentümer (Zufluss-/Abfluss)
└── BetriebskostenabrechnungService → Mieter (Periodengerecht optional)
```

---

## Mieter-Abrechnung (Betriebskostenabrechnung)

### Unterschiede zur Eigentümer-Abrechnung

**Was die Mieter-Abrechnung NICHT enthält:**
- ❌ Nicht umlagefähige Kosten (Verwaltung, Instandhaltung)
- ❌ Rücklagenzuführung
- ❌ Vermögensbericht
- ❌ Hausgeld-Soll/Ist Gegenüberstellung
- ❌ Abrechnungsspitze (WEG-Begriff)

**Was die Mieter-Abrechnung enthält:**
- ✅ Nur umlagefähige Kosten (nach BetrKV)
- ✅ Vorauszahlungen des Mieters
- ✅ Nachzahlung/Guthaben
- ✅ §35a EStG (anteilig)
- ✅ Optional: Periodengerechte Kostenglättung

### Periodengerechte Abrechnung (Mieter)

**Problem bei Zufluss-/Abfluss:**
```
Jahr 2024: Strom 1.000 € (Nachzahlung Vorjahr enthalten)
Jahr 2025: Strom     0 € (Guthaben verrechnet)
→ Mieter zahlt 2024 alles, 2025 nichts
```

**Lösung: Periodengerechte Glättung:**
```
Jahr 2024: Strom 500 € (tatsächlicher Verbrauch 2024)
Jahr 2025: Strom 500 € (tatsächlicher Verbrauch 2025)
→ Faire Verteilung auf beide Jahre
```

### Template: Mieter-Abrechnung

```html
{# templates/betriebskostenabrechnung/pdf.html.twig #}

<h1>BETRIEBSKOSTENABRECHNUNG {{ year }}</h1>
<p><em>Für Mieter der Wohnung {{ einheit.bezeichnung }}</em></p>

<h2>Umlagefähige Betriebskosten</h2>
<table>
    <tr>
        <th>Kostenart (nach BetrKV)</th>
        <th>Gesamt</th>
        <th>Ihr Anteil</th>
    </tr>
    ...
    <tr>
        ...
        <td>{{ item.gesamtkosten|number_format(2, ',', '.') }} €</td>
        <td>{{ item.anteil|number_format(2, ',', '.') }} €</td>
    </tr>
    {% endfor %}
    <tr class="total">
        <td><strong>Summe Betriebskosten</strong></td>
        <td><strong>{{ reportData.totalUmlagefaehig|number_format(2, ',', '.') }} €</strong></td>
        <td><strong>{{ reportData.anteilUmlagefaehig|number_format(2, ',', '.') }} €</strong></td>
    </tr>
</table>
...
<table>
    <tr>
        <td>Ihre Betriebskosten:</td>
        <td>{{ reportData.anteilUmlagefaehig|number_format(2, ',', '.') }} €</td>
    </tr>
    <tr>
        <td>Ihre Vorauszahlungen:</td>
        <td>{{ reportData.mieterVorauszahlung|number_format(2, ',', '.') }} €</td>
    </tr>
    <tr class="result">
        <td><strong>{{ reportData.saldo > 0 ? 'Nachzahlung' : 'Guthaben' }}:</strong></td>
        <td><strong>{{ reportData.saldo|abs|number_format(2, ',', '.') }} €</strong></td>
    </tr>
</table>

{# Kein Vermögensbericht, keine Rücklagen, keine nicht-umlagefähigen Kosten #}
```

### Datenstruktur Mieter-Abrechnung

```php
// BetriebskostenabrechnungService.php

public function generateReport(WegEinheit $einheit, int $year): array
{
    // NUR umlagefähige Kosten
    $umlagefaehig = $this->getUmlagefaehigeKosten($einheit, $year);

    // Optional: Periodengerechte Glättung
    if ($this->configService->usePeriodengerecht($einheit)) {
        $umlagefaehig = $this->glaetteKosten($umlagefaehig, $year);
    }

    // Mieter-Vorauszahlungen (aus Mietvertrag, nicht WEG-Hausgeld)
    $vorauszahlung = $this->getMieterVorauszahlung($einheit, $year);

    return [
        'umlagefaehigeKosten' => $umlagefaehig,
        'totalUmlagefaehig' => array_sum(array_column($umlagefaehig, 'gesamtkosten')),
        'anteilUmlagefaehig' => array_sum(array_column($umlagefaehig, 'anteil')),
        'mieterVorauszahlung' => $vorauszahlung,
        'saldo' => $this->calculateSaldo($umlagefaehig, $vorauszahlung),
        // KEINE Rücklagen, KEIN Vermögensbericht
    ];
}
```
...
## Grundprinzip: Zufluss-/Abfluss-Prinzip (nur Eigentümer!)
...
### Phase 1: Architektur (Neue Abrechnungstypen)

| Prio | Aufgabe | Aufwand | Nutzen |
|------|---------|---------|--------|
| 1 | Route mit `{type}` Parameter | Klein | Hoch |
| 2 | `BetriebskostenabrechnungService` erstellen | Mittel | Hoch |
| 3 | Mieter-Template (nur umlagefähig) | Mittel | Hoch |
| 4 | Periodengerechte Glättung (optional) | Mittel | Mittel |

### Phase 2: Verbesserungen Eigentümer-Abrechnung
...
## Empfehlung: Zwei Abrechnungstypen

### URL-Struktur

```
/abrechnung/{year}/preview/{einheit}/eigentuemer  → Hausgeldabrechnung
/abrechnung/{year}/preview/{einheit}/mieter       → Betriebskostenabrechnung
/abrechnung/{year}/preview/{einheit}              → Default: eigentuemer
```

### Vorteile dieser Architektur

1. **Klare Trennung** der rechtlichen Anforderungen
2. **Wiederverwendung** der Kostendaten (gleiche Quelle, unterschiedliche Darstellung)
3. **Vermieter-freundlich** - können direkt Mieter-Abrechnung generieren
4. **Rechtssicher** - jede Abrechnung folgt ihrem Rechtsrahmen
5. **Erweiterbar** - weitere Typen möglich (z.B. Steuerberater-Export)

### Nächste Schritte

1. [ ] Route mit `{type}` Parameter implementieren
2. [ ] `BetriebskostenabrechnungService` Interface definieren
3. [ ] Mieter-Template erstellen (nur umlagefähig)
4. [ ] Periodengerechte Option als Config
5. [ ] UI: Dropdown zur Auswahl des Abrechnungstyps
